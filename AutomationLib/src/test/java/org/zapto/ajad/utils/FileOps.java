...
public  class FileOps extends CommonParams {
	/*Str
	 * Obj - Should read the file, and out put a string
	 * 		Get the path from param
	 * 			Define vars to hold data
	 * 
	 * 			Read the file using a buffered reader
	 *		 		
	 * Get data in to buffer
	 * Out put the string
	 */
	...
	//public StringBuilder ReadFile(String ParStrDataFilePath, String ParStrDataCsv ) throws IOException{
		...
		//String StrDataFilePath = PropertyLoader.LoadProperty(ParStrDataFilePath);
	String StrDataCsv= PropertyLoader.LoadProperty(ParStrDataCsv);

		
		//Get the files path as defined in the application properties
		//Path ObjPath = FileSystems.getDefault().getPath(StrDataFilePath, StrDataCsv );
		//Path ObjPath = Paths.get(URI.create(StrDataCsv));
		//Path OPath = Paths.get("/AutomationLib/src/test/resources/datalogin.csv");
		
		//Path ObjPath = FileSystems.getDefault().getPath("src/test/resources/datalogin.csv");
		Path ObjPath = FileSystems.getDefault().getPath(StrDataCsv);
		//Path ObjPath = FileSystems.getDefault().getPath(ParStrDataCsv);
		...
		//Read the file
		BufferedReader ObjBufferedReader = Files.newBufferedReader(ObjPath,ObjCharset);
...
/*	public static void main(String[] args) throws IOException{
		...
		FileOps ObjFileOps = new FileOps();
			//StringBuilder ObjStringBuilder =ObjFileOps.ReadFile("testResource
		...
		StringBuilder ObjStringBuilder =ObjFileOps.ReadFile("datalogin.csv");  
			System.out.println("FileOps.ReadFile() - "+ ObjStringBuilder.toString());
