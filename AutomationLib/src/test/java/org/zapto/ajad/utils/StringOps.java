...
public class StringOps extends CommonParams
	{
		...
		/*
		 * This method with take in the csv data as a string and out put a 2d
		 * array*Fin:)
		 */
		...
		/**
		 * @param strCsvDelimiter
		 *            the strCsvDelimiter to set
		 */
		...
		/**
		 * @param intArr2dFieldCount
		 *            the intArr2dFieldCount to set
		 */
		...
		/**
		 * @param arrDataProvider
		 *            the arrDataProvider to set
		 */
		...
		/**
		 * @param intArr2dRowCount
		 *            the intArr2dRowCount to set
		 */
		...
		public String[][] FormatCsvDataToArray(String ParStrCsvData,
				String ParStrDelimiter, int IntArr2dFieldCount)
			{
				...
				if (ParStrCsvData != null)
					{
						...
						this.setArrStr(ParStrCsvData.split(this
								.getStrCsvDelimiter()));

						IntArr1dRowCount = ArrStr.length;
						this.setIntArr2dRowCount(ArrStr.length
								/ this.getIntArr2dFieldCount());
						...
						for (Integer i = 0, k = 0; i < IntArr1dRowCount; i++, k++)
							{
								for (int j = 0; j < IntArr2dFieldCount; j++, i++)
									{
										// k remains constant after this till
										// the loop ends
										// i is entirely controlled just in this
										// loop
				...
				/*
				 * [Debug] for(int x=0; x <Arr2dStr.length; x++){ for(int
				 * y=0;y<this.getIntArr2dFieldCount(); y++){
		 ...
		 * 
		 * String ParStrCsvData = "a,b,c,d,e,f"; String ParStrDelimiter = ",";
		 * int IntArr2dFieldCount = 2;
		 * 
		 * StringOps ObjStringOps = new StringOps(); String[][] ArrCsvData =
		 * ObjStringOps.FormatCsvDataToArray(ParStrCsvData, ParStrDelimiter,
		 * IntArr2dFieldCount);
		 * 
		 * for(int x=0; x <ArrCsvData.length; x++){ for(int
		 * y=0;y<ArrCsvData[0].length; y++){ System.out.println("ArrCsvData - "
		 * + " [ x ] - "+ x + " [ y ] - " + y + " - " + ArrCsvData[x][y]); } } }
		 */
	}
